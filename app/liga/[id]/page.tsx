...
function winsFromSets(sets:Array<{p1:number,p2:number}>|null):{p1:number,p2:number}{
  ...
  const p1=sets.filter(s=>s.p1>s.p2).length
  const p2=sets.filter(s=>s.p2>s.p1).length
  return {p1,p2}
}
...
export default function SeasonPage({params}:{params:{id:string}}){
  ...
  const [tab,setTab]=useState<"tabelle"|"spielplan"|"feed">("tabelle")
  ...
  useEffect(()=>{
    async function load(){
      ...
      // Matches
      const {data:m}=await sb.from("league_matches").select(`id,round,group_number,p1_id,p2_id,sets,winner_id,status,deadline,played_at,p1:profiles!league_matches_p1_id_fkey(name),p2:profiles!league_matches_p2_id_fkey(name)`).eq("season_id",params.id).order("round",{ascending:true})
      const mapped=(m||[]).map((x:Record<string,unknown>)=>({...x,p1_name:(x.p1 as {name:string}|null)?.name||"?",p2_name:(x.p2 as {name:string}|null)?.name||"?"}))
      setMatches(mapped as Match[])

      // Feed: confirmed matches with reactions + comments
      const {data:f}=await sb.from("league_matches").select(`id,round,p1_id,p2_id,sets,winner_id,played_at,p1:profiles!league_matches_p1_id_fkey(name),p2:profiles!league_matches_p2_id_fkey(name),match_reactions(type,user_id),match_comments(id,user_id,text,created_at,profiles(name))`).eq("season_id",params.id).eq("status","confirmed").order("played_at",{ascending:false}).limit(20)
      const feedMapped=(f||[]).map((x:Record<string,unknown>)=>({
        ...x,
        p1_name:(x.p1 as {name:string}|null)?.name||"?",
        p2_name:(x.p2 as {name:string}|null)?.name||"?",
  ...
  async function handleReact(matchId:string,type:string){
    ...
    const {data}=await sb2.from("match_reactions").select("type,user_id").eq("match_id",matchId)
    ...
    <main style={{minHeight:"100vh",background:BG,padding:"20px 20px 80px"}}>
      <div style={{maxWidth:560,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",background:C,borderRadius:10,padding:3,marginBottom:16,gap:2}}>
          {(["tabelle","spielplan","feed"] as const).map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{flex:1,background:tab===t?BG:"none",border:"none",borderRadius:8,padding:"9px 4px",fontSize:12,fontWeight:700,color:tab===t?W:M,cursor:"pointer",textTransform:"uppercase",letterSpacing:"0.06em",transition:"all 0.2s"}}>
              {t==="tabelle"?"Tabelle":t==="spielplan"?"Spielplan":"Feed"}
            </button>
          ))}
        </div>
        ...
        {tab==="tabelle"&&(
          <div style={{background:C,border:`1px solid ${B}`,borderRadius:14,overflow:"hidden"}}>
            {/* Header */}
            <div style={{display:"grid",gridTemplateColumns:"28px 1fr 36px 60px",padding:"10px 14px",borderBottom:`1px solid ${B}`,background:"#0D0E12"}}>
            ...
            {standings.length===0?(
              ...
              <div key={s.player_id} style={{display:"grid",gridTemplateColumns:"28px 1fr 36px 60px",padding:"12px 14px",borderBottom:i<standings.length-1?`1px solid ${B}`:"none",background:s.player_id===userId?`${G}08`:"none",alignItems:"center"}}>
                <span style={{fontSize:13,fontWeight:i<3?700:400,color:i===0?G:i===1?"#A8AAB2":i===2?"#CD7F32":M}}>{i+1}</span>
                <div>
                  <span style={{fontSize:14,fontWeight:700,color:s.player_id===userId?G:W}}>{s.player_name}</span>
                  {s.player_id===userId&&<span style={{fontSize:10,color:G,marginLeft:6}}>du</span>}
                </div>
        ...
        {tab==="spielplan"&&(
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {matches.length===0?<p style={{textAlign:"center",color:M,padding:"20px"}}>Noch kein Spielplan</p>:
            matches.map(m=>{
              ...
              return(
                <div key={m.id} style={{background:C,border:`1px solid ${isMe?`${G}40`:B}`,borderRadius:12,padding:"14px 16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    <span style={{fontSize:10,color:M,fontWeight:700,textTransform:"uppercase"}}>Runde {m.round}</span>
                    <span style={{fontSize:10,color:confirmed?G:m.status==="p1_entered"?"#FACC15":M,fontWeight:700,textTransform:"uppercase"}}>{confirmed?"✓ Bestätigt":m.status==="p1_entered"?"Warten auf Bestätigung":"Ausstehend"}</span>
                  </div>
