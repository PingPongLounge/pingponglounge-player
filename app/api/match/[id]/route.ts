...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: game } = await sb
    .from("open_games")
    .select("id,created_by,location_name,date,start_hour,duration_minutes,max_players,current_players,price_per_player,level,status,notes,created_at,winner_id,entered_by,sets,entered_at,confirmed_at")
    .eq("id", id)
    .single()
