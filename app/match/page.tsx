      ...
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", margin: "20px 0 20px" }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: M, marginTop: 8, fontWeight: 400 }}>{games.length} offen · {filtered.length} angezeigt</p>
          </div>
          {!myGame ? (
        ...
        {loading ? (
          ...
          <div style={{ background: C, border: `1px solid ${B}`, borderRadius: 16, padding: "40px 20px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: M, marginBottom: 20, fontWeight: 400 }}>erstell das erste!</p>
              ...
              return (
                <div key={g.id} style={{ background: C, border: `1px solid ${isMe ? "rgba(255,255,255,0.25)" : B}`, borderRadius: 16, overflow: "hidden" }}>
                  <div style={{ padding: "14px 16px 10px" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                          ...
                          <span style={{ fontSize: 11, color: M, fontWeight: 400 }}>elo {host?.elo ?? "—"}</span>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          ...
                          <span style={{ fontSize: 11, color: M, background: "#1A1D24", borderRadius: 999, padding: "2px 8px", fontWeight: 400, textTransform: "lowercase" }}>📍 {g.location_name}</span>
                          <span style={{ fontSize: 11, color: M, background: "#1A1D24", borderRadius: 999, padding: "2px 8px", fontWeight: 400, textTransform: "lowercase" }}>🕐 {whenLabel(g.date, g.start_hour)}</span>
                          {g.price_per_player > 0
                            ? <span style={{ fontSize: 11, color: M, background: "#1A1D24", borderRadius: 999, padding: "2px 8px", fontWeight: 400, textTransform: "lowercase" }}>💰 chf {g.price_per_player}</span>
                            : <span style={{ fontSize: 11, color: G, background: "#1A1D24", borderRadius: 999, padding: "2px 8px", fontWeight: 400, textTransform: "lowercase" }}>gratis</span>}
                        </div>
                      </div>
                    ...
                    {g.notes && <p style={{ fontSize: 12, color: M, fontStyle: "italic", fontWeight: 400, margin: "2px 0 4px" }}>&quot;{g.notes}&quot;</p>}
                  </div>
                  <div style={{ padding: "4px 16px 14px" }}>
                    {isMe || joined ? (
                      ...
                      <p style={{ fontSize: 12, color: M, textAlign: "center", fontWeight: 400, textTransform: "lowercase" }}>voll</p>
                    ) : (
