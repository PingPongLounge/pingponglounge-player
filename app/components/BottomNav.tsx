...
const BG = "#08090B" // fast schwarz, passend zum Header
          ...
          return (
            <Link key={t.href} href={t.href} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5, textDecoration: "none" }}>
              <span style={{ position: "relative", display: "inline-flex" }}>
                {t.icon === "home" ? (
                  <svg width="29" height="29" viewBox="0 0 24 24" fill="none" stroke={active ? "#57CF79" : MUT} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : .6 }}>
                ...
                {t.href === "/liga" && pending > 0 && (
                  <span style={{ position: "absolute", top: -4, right: -7, minWidth: 17, height: 17, borderRadius: 999, background: "linear-gradient(135deg,#57CF79,#38BEB2)", color: "#06210F", fontSize: 10, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", boxShadow: "0 0 0 2px #1A1E25" }}>{pending > 9 ? "9+" : pending}</span>
                )}
              </span>
