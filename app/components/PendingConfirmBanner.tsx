...
// Hinweis oben: "Ein Ergebnis wartet auf deine Bestätigung." Ohne das merkte
// niemand, dass der Gegner ein Resultat eingetragen hat — man musste zufällig
// in die Liga schauen. Ein Tap führt direkt zum Bestätigen.
export default function PendingConfirmBanner() {
  ...
  const title = one
    ? `${one.opponent} hat ein Ergebnis eingetragen`
    : `${items.length} Ergebnisse warten auf dich`
  const sub = one
    ? `${one.scoreLine} für dich · bitte bestätigen`
    : "Bitte bestätige deine Spiele"
  ...
  return (
    <button
      ...
      style={{
        ...
        background: "rgba(57,255,20,.08)", border: "1px solid rgba(57,255,20,.38)",
        borderRadius: 16, padding: "13px 15px", margin: "0 0 14px",
      }}
    >
      <span style={{
        width: 40, height: 40, borderRadius: "50%", flexShrink: 0,
        background: "linear-gradient(135deg,#39FF14,#1FD1C4)", color: "#06210F",
      ...
      <span style={{
        ...
        color: "#06210F", background: "linear-gradient(135deg,#39FF14,#1FD1C4)",
